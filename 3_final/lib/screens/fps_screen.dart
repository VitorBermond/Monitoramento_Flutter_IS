...
class _FPSScreenState extends State<FPSScreen> {
  ...
  final fpsDataStore = MetricDataManager.getInstance("FPS");

  /// Variáveis Auxiliares
  static const String titulo = "Monitoramento de Frames por segundo (FPS)";

  // Conexão RabbitMQ
  late Client client;
  static const String realQueue = "Data.FPS";
  static const String histQueue = "Hist.FPS";
  static const String reqHQueue = "HistRequest.FPS";

  // Construção do gráfico
  static const String unit = "fps";
  static const double cMinY = 0;
  static const Null cMaxY = null;

  // JSON
  static const String metric = "FPS";
      ...
      queue.consume().then((Consumer consumer) {
        consumer.listen((AmqpMessage message) {
          ...
          setState(() {
            ...
            if (!fpsDataStore.servicesList.containsValue(serviceName)) {
              int newKey = fpsDataStore.servicesList.length;
              fpsDataStore.servicesList[newKey] = serviceName;
            }

            // Inicializa os mapas, usando serviceName como chave
            fpsDataStore.realData.putIfAbsent(serviceName, () => []);
            fpsDataStore.timeLabelsReal.putIfAbsent(serviceName, () => []);
            fpsDataStore.timeEpochsReal.putIfAbsent(serviceName, () => []);

            // Remove o primeiro ponto se exceder o limite
            if ((fpsDataStore.realData[serviceName]?.length ?? 0) >=
                maxPoints) {
              fpsDataStore.realData[serviceName]?.removeAt(0);
              fpsDataStore.timeLabelsReal[serviceName]?.removeAt(0);
              fpsDataStore.timeEpochsReal[serviceName]?.removeAt(0);
            }

            // Adiciona os dados ao gráfico e aos rótulos
            fpsDataStore.realData[serviceName]!.add(
              FlSpot(fpsDataStore.indexReal.toDouble(), metricValue),
            );
            fpsDataStore.timeLabelsReal[serviceName]!.add(formattedTime);
            fpsDataStore.timeEpochsReal[serviceName]!.add(timestampEpoch);

            // Incrementa o índice global
            fpsDataStore.indexReal++;
          });
        });
      });
    } catch (e) {
          ...
          try {
            ...
            if (payload is Map<String, dynamic> &&
              ...
              if (data is List<dynamic>) {
                setState(() {
                  if (!fpsDataStore.servicesList.containsValue(serviceName)) {
                    final newKey = fpsDataStore.servicesList.length;
                    fpsDataStore.servicesList[newKey] = serviceName;
                  }

                  fpsDataStore.histData.putIfAbsent(serviceName, () => []);
                  fpsDataStore.histData[serviceName]?.clear();
                  fpsDataStore.timeLabelsHist
                      .putIfAbsent(serviceName, () => []);
                  fpsDataStore.timeLabelsHist[serviceName]?.clear();
                  fpsDataStore.timeEpochsHist
                      .putIfAbsent(serviceName, () => []);
                  fpsDataStore.timeEpochsHist[serviceName]?.clear();
                  ...
                  for (var entry in data) {
                    if (entry is Map<String, dynamic> &&
                      ...
                      fpsDataStore.histData[serviceName]!.add(FlSpot(
                        timeIndex.toDouble(),
                      ...
                      fpsDataStore.timeLabelsHist[serviceName]!
                          .add(formattedTime);
                      fpsDataStore.timeEpochsHist[serviceName]!
                          .add(timestampEpoch);
  ...
  void _applyDownsampling(String serviceName) {
    if (!fpsDataStore.histData.containsKey(serviceName) ||
        fpsDataStore.histData[serviceName]!.length <= maxPoints) {
      return;
    }

    final List<FlSpot> originalData = fpsDataStore.histData[serviceName]!;
    ...
    fpsDataStore.histData[serviceName] = downsampledData;
    fpsDataStore.timeLabelsHist[serviceName] = downsampledLabels;
    fpsDataStore.timeEpochsHist[serviceName] = downsampledEpochs;
  }
      ...
      channel.then((Channel ch) {
        ch.queue(reqHQueue, durable: false).then((Queue queue) {
          final request = jsonEncode({
            ...
            "services_list": fpsDataStore.servicesList.values.toList(),
          });
  ...
  void _addManualService() {
    ...
    setState(() {
      ...
      if (!fpsDataStore.servicesList.containsValue(name)) {
        final newKey = fpsDataStore.servicesList.length;
        fpsDataStore.servicesList[newKey] = name;
      }

      // garante que todos os mapas já existam (tratamento não nulo)
      fpsDataStore.realData.putIfAbsent(name, () => []);
      fpsDataStore.timeLabelsReal.putIfAbsent(name, () => []);
      fpsDataStore.timeEpochsReal.putIfAbsent(name, () => []);
      fpsDataStore.histData.putIfAbsent(name, () => []);
      fpsDataStore.timeLabelsHist.putIfAbsent(name, () => []);
      fpsDataStore.timeEpochsHist.putIfAbsent(name, () => []);
    });
