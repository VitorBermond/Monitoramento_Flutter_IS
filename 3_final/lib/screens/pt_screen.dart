...
class _PTScreenState extends State<PTScreen> {
  ...
  final ptDataStore = MetricDataManager.getInstance("PT");

  /// Variáveis Auxiliares
  static const String titulo = "Monitoramento do tempo de processamento";

  // Conexão RabbitMQ
  late Client client;
  static const String realQueue = "Data.PT";
  static const String histQueue = "Hist.PT";
  static const String reqHQueue = "HistRequest.PT";

  // Construção do gráfico
  static const String unit = "ms";
  static const double cMinY = 0;
  static const Null cMaxY = null;

  // JSON
  // Adaptado para formato bruto do zipkin
  static const String tags = 'tags';
  static const String metric = "Tempo de Processamento (ms)";
  static const String timestamp = 'timestamp';
  static const String localEndpoint = 'localEndpoint';
    ...
    try {
      ...
      queue.consume().then((Consumer consumer) {
        consumer.listen((AmqpMessage message) {
          ...
          double metricValue = (data[tags][metric] as num).toDouble(); // Ex: 50
          String serviceName = (data.containsKey(localEndpoint) &&
                  data[localEndpoint]?[servicename]
                          ?.toString()
                          .trim()
                          .isNotEmpty ==
                      true)
              ? data[localEndpoint]![servicename].toString()
              : 'APP'; // fallback
          ...
          setState(() {
            ...
            if (!ptDataStore.servicesList.containsValue(serviceName)) {
              int newKey = ptDataStore.servicesList.length;
              ptDataStore.servicesList[newKey] = serviceName;
            }

            // Inicializa os mapas, usando serviceName como chave
            ptDataStore.realData.putIfAbsent(serviceName, () => []);
            ptDataStore.timeLabelsReal.putIfAbsent(serviceName, () => []);
            ptDataStore.timeEpochsReal.putIfAbsent(serviceName, () => []);

            // Remove o primeiro ponto se exceder o limite
            if ((ptDataStore.realData[serviceName]?.length ?? 0) >= maxPoints) {
              ptDataStore.realData[serviceName]?.removeAt(0);
              ptDataStore.timeLabelsReal[serviceName]?.removeAt(0);
              ptDataStore.timeEpochsReal[serviceName]?.removeAt(0);
            }

            // Adiciona os dados ao gráfico e aos rótulos
            ptDataStore.realData[serviceName]!.add(
              FlSpot(ptDataStore.indexReal.toDouble(), metricValue),
            );
            ptDataStore.timeLabelsReal[serviceName]!.add(formattedTime);
            ptDataStore.timeEpochsReal[serviceName]!.add(timestampEpoch);

            // Incrementa o índice global
            ptDataStore.indexReal++;
          });
        });
      });
    } catch (e) {
          ...
          try {
            ...
            if (payload is Map<String, dynamic> &&
              ...
              if (data is List<dynamic>) {
                setState(() {
                  if (!ptDataStore.servicesList.containsValue(serviceName)) {
                    final newKey = ptDataStore.servicesList.length;
                    ptDataStore.servicesList[newKey] = serviceName;
                  }

                  ptDataStore.histData.putIfAbsent(serviceName, () => []);
                  ptDataStore.histData[serviceName]?.clear();
                  ptDataStore.timeLabelsHist.putIfAbsent(serviceName, () => []);
                  ptDataStore.timeLabelsHist[serviceName]?.clear();
                  ptDataStore.timeEpochsHist.putIfAbsent(serviceName, () => []);
                  ptDataStore.timeEpochsHist[serviceName]?.clear();
                  ...
                  for (var entry in data) {
                    if (entry is Map<String, dynamic> &&
                      ...
                      ptDataStore.histData[serviceName]!.add(FlSpot(
                        timeIndex.toDouble(),
                      ...
                      ptDataStore.timeLabelsHist[serviceName]!
                          .add(formattedTime);
                      ptDataStore.timeEpochsHist[serviceName]!
                          .add(timestampEpoch);
  ...
  void _applyDownsampling(String serviceName) {
    if (!ptDataStore.histData.containsKey(serviceName) ||
        ptDataStore.histData[serviceName]!.length <= maxPoints) {
      return;
    }

    final List<FlSpot> originalData = ptDataStore.histData[serviceName]!;
    ...
    ptDataStore.histData[serviceName] = downsampledData;
    ptDataStore.timeLabelsHist[serviceName] = downsampledLabels;
    ptDataStore.timeEpochsHist[serviceName] = downsampledEpochs;
  }
      ...
      channel.then((Channel ch) {
        ch.queue(reqHQueue, durable: false).then((Queue queue) {
          final request = jsonEncode({
            ...
            "services_list": ptDataStore.servicesList.values.toList(),
          });
  ...
  void _addManualService() {
    ...
    setState(() {
      ...
      if (!ptDataStore.servicesList.containsValue(name)) {
        final newKey = ptDataStore.servicesList.length;
        ptDataStore.servicesList[newKey] = name;
      }

      // garante que todos os mapas já existam (tratamento não nulo)
      ptDataStore.realData.putIfAbsent(name, () => []);
      ptDataStore.timeLabelsReal.putIfAbsent(name, () => []);
      ptDataStore.timeEpochsReal.putIfAbsent(name, () => []);
      ptDataStore.histData.putIfAbsent(name, () => []);
      ptDataStore.timeLabelsHist.putIfAbsent(name, () => []);
      ptDataStore.timeEpochsHist.putIfAbsent(name, () => []);
    });
