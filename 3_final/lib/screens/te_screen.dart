...
class TEScreen extends StatefulWidget {
  const TEScreen({super.key});

  @override
  State<TEScreen> createState() => _TEScreenState();
}

class _TEScreenState extends State<TEScreen> {
  // Cria o DataStore (metrics.dart)
  final teDataStore = MetricDataManager.getInstance("TE");

  /// Variáveis Auxiliares
  static const String titulo = "Monitoramento do Erro de Trajetória";

  // Conexão RabbitMQ
  late Client client;
  static const String realQueue = "Data.TE";
  static const String histQueue = "Hist.TE";
  static const String reqHQueue = "HistRequest.TE";
  ...
  // JSON
  static const String metric = "trajectory_error";
      ...
      queue.consume().then((Consumer consumer) {
        consumer.listen((AmqpMessage message) {
          ...
          setState(() {
            ...
            if (!teDataStore.servicesList.containsValue(serviceName)) {
              int newKey = teDataStore.servicesList.length;
              teDataStore.servicesList[newKey] = serviceName;
            }

            // Inicializa os mapas, usando serviceName como chave
            teDataStore.realData.putIfAbsent(serviceName, () => []);
            teDataStore.timeLabelsReal.putIfAbsent(serviceName, () => []);
            teDataStore.timeEpochsReal.putIfAbsent(serviceName, () => []);

            // Remove o primeiro ponto se exceder o limite
            if ((teDataStore.realData[serviceName]?.length ?? 0) >= maxPoints) {
              teDataStore.realData[serviceName]?.removeAt(0);
              teDataStore.timeLabelsReal[serviceName]?.removeAt(0);
              teDataStore.timeEpochsReal[serviceName]?.removeAt(0);
            }

            // Adiciona os dados ao gráfico e aos rótulos
            teDataStore.realData[serviceName]!.add(
              FlSpot(teDataStore.indexReal.toDouble(), metricValue),
            );
            teDataStore.timeLabelsReal[serviceName]!.add(formattedTime);
            teDataStore.timeEpochsReal[serviceName]!.add(timestampEpoch);

            // Incrementa o índice global
            teDataStore.indexReal++;
          });
        });
      });
    } catch (e) {
          ...
          try {
            ...
            if (payload is Map<String, dynamic> &&
              ...
              if (data is List<dynamic>) {
                setState(() {
                  if (!teDataStore.servicesList.containsValue(serviceName)) {
                    final newKey = teDataStore.servicesList.length;
                    teDataStore.servicesList[newKey] = serviceName;
                  }

                  teDataStore.histData.putIfAbsent(serviceName, () => []);
                  teDataStore.histData[serviceName]?.clear();
                  teDataStore.timeLabelsHist.putIfAbsent(serviceName, () => []);
                  teDataStore.timeLabelsHist[serviceName]?.clear();
                  teDataStore.timeEpochsHist.putIfAbsent(serviceName, () => []);
                  teDataStore.timeEpochsHist[serviceName]?.clear();
                  ...
                  for (var entry in data) {
                    if (entry is Map<String, dynamic> &&
                      ...
                      teDataStore.histData[serviceName]!.add(FlSpot(
                        timeIndex.toDouble(),
                      ...
                      teDataStore.timeLabelsHist[serviceName]!
                          .add(formattedTime);
                      teDataStore.timeEpochsHist[serviceName]!
                          .add(timestampEpoch);
  ...
  void _applyDownsampling(String serviceName) {
    if (!teDataStore.histData.containsKey(serviceName) ||
        teDataStore.histData[serviceName]!.length <= maxPoints) {
      return;
    }

    final List<FlSpot> originalData = teDataStore.histData[serviceName]!;
    ...
    teDataStore.histData[serviceName] = downsampledData;
    teDataStore.timeLabelsHist[serviceName] = downsampledLabels;
    teDataStore.timeEpochsHist[serviceName] = downsampledEpochs;
  }
      ...
      channel.then((Channel ch) {
        ch.queue(reqHQueue, durable: false).then((Queue queue) {
          final request = jsonEncode({
            ...
            "services_list": teDataStore.servicesList.values.toList(),
          });
  ...
  void _addManualService() {
    ...
    setState(() {
      ...
      if (!teDataStore.servicesList.containsValue(name)) {
        final newKey = teDataStore.servicesList.length;
        teDataStore.servicesList[newKey] = name;
      }

      // garante que todos os mapas já existam (tratamento não nulo)
      teDataStore.realData.putIfAbsent(name, () => []);
      teDataStore.timeLabelsReal.putIfAbsent(name, () => []);
      teDataStore.timeEpochsReal.putIfAbsent(name, () => []);
      teDataStore.histData.putIfAbsent(name, () => []);
      teDataStore.timeLabelsHist.putIfAbsent(name, () => []);
      teDataStore.timeEpochsHist.putIfAbsent(name, () => []);
    });
