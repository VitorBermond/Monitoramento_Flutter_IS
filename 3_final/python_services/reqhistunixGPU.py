...
channel.queue_declare(queue='HistRequest.GPU', durable=False)
channel.queue_declare(queue='Hist.GPU', durable=False)
...
def callback(ch, method, properties, body):
    try:
        ...
        for service_name in services:
            filename = f"gpu_usage_{service_name}.csv"
...
channel.basic_consume(queue='HistRequest.GPU', on_message_callback=callback, auto_ack=True)
